...
import {computed, ref, Ref, ComputedRef, onMounted, watch} from "vue";
...
onMounted(async () => {
  ...
  if (curType.value !== 'all_objects') {
    data.value = data.value.filter((item: PropertyCatalogType) => item.type === curType.value);
  }
})
...
const renderTitle = () => {
  if (props.title === 'Виллы' || props.title === 'Новостройки') {
    return false
  } else {
    return true
  }
...
<template>
  <section class="catalog">
    <div v-if="renderTitle()" class="catalog__filter filter">
    ...
    <div class="catalog__content container">
      ...
      <div v-if="renderTitle()" class="catalog__cards-filter cards-filter">
      ...
      <cards-list :url="URL" :locationType="locationType"/>
    </div>
  </section>
  <form-sub/>
</template>
